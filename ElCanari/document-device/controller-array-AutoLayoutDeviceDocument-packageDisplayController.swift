...
//--------------------------------------------------------------------------------------------------
//    Array controller AutoLayoutDeviceDocument packageDisplayController
//--------------------------------------------------------------------------------------------------

final class Controller_AutoLayoutDeviceDocument_packageDisplayController : EBReadOnlyAbstractGenericRelationshipProperty, EBGraphicViewControllerProtocol, HiddenEBProtocol {
  ...
  private var mPrivateSelectedSet = EBReferenceSet <PackageInDevice> () {
    didSet {
  ...
  @objc func remove (_ _ : Any) {
    if let model = self.mModel {
      ...
      //----------------------------------------- Set new selection
        var newSelectionSet = EBReferenceSet <PackageInDevice> ()
   ...
   func pasteFromPasteboard (_ inPasteboardType : NSPasteboard.PasteboardType?, _ inWindow : NSWindow) {
    ...
    if let pasteboardType = inPasteboardType,
      ...
      for dictionary in dictionaryArray {
        idx += 1
        if let object = makeManagedObjectFromDictionary (self.undoManager, dictionary) as? PackageInDevice {
 ...
 // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  func sendToBack () {
    var objects = self.objectArray
    let sortedIndexArray = self.sortedIndexArrayOfSelectedObjects ()
    var newObjectArray = EBReferenceArray <PackageInDevice> ()
    for idx in sortedIndexArray.reversed () {
      let object = objects [idx]
      objects.remove (at: idx)
...
/*  func removeFromSelection (objectWithIndex inIndex : Int) {
  ...
  func setSelection (objectsWithIndexes inIndexes : [Int]) {
    let objects = self.objectArray
    var selectedObjects = [PackageInDevice] ()
