...
final class Controller_AutoLayoutDeviceDocument_symbolDisplayController : EBReadOnlyAbstractGenericRelationshipProperty, EBGraphicViewControllerProtocol, HiddenEBProtocol {
  ...
  var selectedArray : EBReferenceArray <SymbolInstanceInDevice> { return self.selectedArray_property.propval }
  ...
  final func bind_model (_ inModel : ReadWriteArrayOf_SymbolInstanceInDevice, _ inUndoManager : UndoManager?) {
    self.mModel = inModel
  ...
  @objc func remove (_ _ : Any) {
    if let model = self.mModel {
      ...
      //----------------------------------------- Set new selection
        var newSelectionSet = EBReferenceSet <SymbolInstanceInDevice> ()
 ...
 // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  func sendToBack () {
    var objects = self.objectArray
    let sortedIndexArray = self.sortedIndexArrayOfSelectedObjects ()
    var newObjectArray = EBReferenceArray <SymbolInstanceInDevice> ()
    for idx in sortedIndexArray.reversed () {
      let object = objects [idx]
      objects.remove (at: idx)
 ...
 // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  func addToSelection (objects inObjects : [SymbolInstanceInDevice]) {
