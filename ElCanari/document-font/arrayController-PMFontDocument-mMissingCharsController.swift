...
//——————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
//    SelectedSet_PMFontDocument_mMissingCharsController
//——————————————————————————————————————————————————————————————————————————————————————————————————————————————————————

final class SelectedSet_PMFontDocument_mMissingCharsController : EBAbstractProperty {
  ...
  init (allowsEmptySelection : Bool,
        ...
        sortedArray : TransientArrayOf_MissingCharacter) {
  ...
  private var mPrivateSet = Set<MissingCharacter> () {
    didSet {
...
//——————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
//    ArrayController_PMFontDocument_mMissingCharsController
//——————————————————————————————————————————————————————————————————————————————————————————————————————————————————————

final class ArrayController_PMFontDocument_mMissingCharsController : EBObject, EBTableViewDelegate, EBTableViewDataSource {

  private var mModel : ReadOnlyArrayOf_MissingCharacter? = nil

  let sortedArray = TransientArrayOf_MissingCharacter ()

  let selectedArray = TransientArrayOf_MissingCharacter ()

  private let mSelectedSet : SelectedSet_PMFontDocument_mMissingCharsController
  ...
  override init () {
    mSelectedSet = SelectedSet_PMFontDocument_mMissingCharsController (
  ...
  func isOrderedBefore (left : MissingCharacter, right : MissingCharacter) -> Bool {
    var order = ComparisonResult.orderedSame
    for (column, ascending) in mSortDescriptorArray {
      if column == "code" {
        order = compare_Int (left: left.idx, right:right.idx)
      }
      if !ascending {
        switch order {
  ...
  //--- Add observed properties (for filtering and sorting)
    model.addEBObserverOf_idx (sortedArray)
  //--- Bind table views
  ...
  //--- Remove observed properties (for filtering and sorting)
    mModel?.removeEBObserverOf_idx (sortedArray)
    ...
    case .single (let v) :
      ...
      if columnIdentifier == "code" {
        if let cell : EBTextObserverField_TableViewCell = result as? EBTextObserverField_TableViewCell {
          ...
          cell.mCellOutlet?.bind_valueObserver (object.code, file: #file, line: #line)
        }
        ...
        if let cell : EBTextObserverField_TableViewCell = result as? EBTextObserverField_TableViewCell {
          ...
          cell.mUnbindFunction? ()
          cell.mCellOutlet?.bind_valueObserver (object.char, file: #file, line: #line)
        }
      }else{
