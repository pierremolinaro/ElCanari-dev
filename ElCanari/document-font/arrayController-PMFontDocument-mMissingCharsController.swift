...
final class ArrayController_PMFontDocument_mMissingCharsController : EBObject, EBTableViewDelegate, EBTableViewDataSource {
  ...
  let sortedArray_property = TransientArrayOf_MissingCharacter ()

  let selectedArray_property = TransientArrayOf_MissingCharacter ()
  ...
  func setSelection (_ inObjects : [MissingCharacter]) {
  ...
  func isOrderedBefore (left : MissingCharacter, right : MissingCharacter) -> Bool {
    ...
    for (column, ascending) in mSortDescriptorArray {
      if column == "code" {
        order = compare_Int (left: left.idx_property, right:right.idx_property)
      }
      if !ascending {
        switch order {
