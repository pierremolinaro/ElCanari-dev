...
final class Controller_AutoLayoutFontDocument_selectedCharacterController : EBObjcBaseObject, AutoLayoutTableViewDelegate {
  ...
  private var mSortDescriptorArray = [(FontCharacter, FontCharacter) -> ComparisonResult] ()
  ...
  final func bind_model (_ inModel : ReadWriteArrayOf_FontCharacter, _ inUndoManager : UndoManager?) {
    self.mModel = inModel
  ...
  var selectedSet : EBReferenceSet <FontCharacter> { return EBReferenceSet (self.selectedArray_property.propval.values) }
  ...
  final func select (object inObject: FontCharacter) {
    if let model = self.mModel {
