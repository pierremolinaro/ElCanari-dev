...
final class Controller_AutoLayoutMergerDocument_mDataController : EBObjcBaseObject, AutoLayoutTableViewDelegate {
  ...
  final func bind_model (_ inModel : ReadWriteArrayOf_ArtworkFileGenerationParameters, _ inUndoManager : UndoManager?) {
    self.mModel = inModel
  ...
  final func select (object inObject: ArtworkFileGenerationParameters) {
    if let model = self.mModel {
