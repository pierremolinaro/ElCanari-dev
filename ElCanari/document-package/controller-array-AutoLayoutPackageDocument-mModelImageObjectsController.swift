...
final class Controller_AutoLayoutPackageDocument_mModelImageObjectsController : ReadOnlyAbstractGenericRelationshipProperty, EBGraphicViewControllerProtocol, HiddenEBProtocol {
  ...
  private var mPrivateSelectedSet = EBReferenceSet <PackageModelImageDoublePoint> () {
    didSet {
  ...
  final func bind_model (_ inModel : ReadWriteArrayOf_PackageModelImageDoublePoint, _ inUndoManager : UndoManager?) {
    self.mModel = inModel
   ...
   @objc func add (_ sender : Any) {
    if let model = self.mModel {
      ...
      case .empty, .multiple :
        break
      case .single (let v) :
        let newObject = PackageModelImageDoublePoint (self.undoManager)
  ...
  @objc func remove (_ sender : Any) {
    if let model = self.mModel {
      ...
      //----------------------------------------- Set new selection
        var newSelectionSet = EBReferenceSet <PackageModelImageDoublePoint> ()
   ...
   func pasteFromPasteboard (_ inPasteboardType : NSPasteboard.PasteboardType?, _ inWindow : NSWindow) {
    ...
    if let pasteboardType = inPasteboardType,
      ...
      for dictionary in dictionaryArray {
        idx += 1
        if let object = makeManagedObjectFromDictionary (self.undoManager, dictionary) as? PackageModelImageDoublePoint {
