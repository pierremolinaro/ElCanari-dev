...
final class Controller_AutoLayoutProjectDocument_componentController : EBObjcBaseObject, AutoLayoutTableViewDelegate {
  ...
  final func bind_model (_ inModel : ReadWriteArrayOf_ComponentInProject, _ inUndoManager : UndoManager?) {
    self.mModel = inModel
  ...
  final func select (object inObject: ComponentInProject) {
    if let model = self.mModel {
  ...
  final func addEntry () {
    if let model = self.mModel {
      ...
      case .empty, .multiple :
        ()
      case .single (let v) :
        let newObject = ComponentInProject (self.undoManager)
