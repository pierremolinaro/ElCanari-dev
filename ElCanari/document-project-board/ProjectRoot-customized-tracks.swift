...
extension ProjectRoot {
  ...
  internal func tracks (at inLocation : CanariPoint, trackSide inSide : TrackSide) -> [BoardTrack] {
    ...
    let maxDistance = milsToCanariUnit (double: self.mControlKeyHiliteDiameter)
    for object in self.mBoardObjects {
      if let track = object as? BoardTrack, track.mSide == inSide, let p1 = track.mConnectorP1?.location, let p2 = track.mConnectorP2?.location {
