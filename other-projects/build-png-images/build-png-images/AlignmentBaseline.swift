...
class AlignmentBaselineView : NSView {
...
//      __NSRectFill (inDirtyRect)
    ...
    let y = drawingRect.minY + 0.25 * drawingRect.size.height
    bp.move (to: NSPoint (x: drawingRect.minX, y: y))
    bp.line (to: NSPoint (x: drawingRect.maxX, y: y))
    bp.move (to: NSPoint (x: drawingRect.midX, y: drawingRect.minY))
    bp.line (to: NSPoint (x: drawingRect.midX, y: drawingRect.maxY))
    bp.move (to: NSPoint (x: drawingRect.midX, y: y))
