...
class AlignmentRightView : NSView {
...
//      __NSRectFill (inDirtyRect)
    ...
    let xL = drawingRect.minX + 0.2 * drawingRect.size.width
    ...
    bp.move (to: NSPoint (x: xL, y: y1))
    bp.line (to: NSPoint (x: drawingRect.maxX, y: y1))
    bp.move (to: NSPoint (x: drawingRect.minX, y: y2))
    bp.line (to: NSPoint (x: drawingRect.maxX, y: y2))
    bp.move (to: NSPoint (x: xL, y: y3))
    bp.line (to: NSPoint (x: drawingRect.maxX, y: y3))
    bp.stroke ()
  }
